...
def setup_specific_hand(pbn_hand_notation)
  let(:cards) do
    Bridge::Pbn::DealHandAndEachGame.hand pbn_hand_notation
  end
end
...
RSpec.describe Bridge::Hand do
  ...
  describe '#high_card_points' do
    ...
    context 'with one of each high card rank' do
      let(:cards) { low_cards.sample(9) + jacks.sample(1) + queens.sample(1) + kings.sample(1) + aces.sample(1) }
      it 'returns 10' do
        expect(subject.high_card_points).to eq 10
      end
    end
  end
