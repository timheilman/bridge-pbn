RSpec.describe Bridge::Hand do
  ...
  describe "#short_points" do
    ...
    context "with a trump suit" do
      let(:trump) { Bridge::Suit.card.first }
      let(:cards) do
        lengths = [9,2,1,0]
        suits = Bridge::Suit.card
        suits.map do |suit|
  ...
  describe "#long_points" do
    context "with all hearts" do
      let(:cards) { low_by_suit[Bridge::Suit::Heart] + high_by_suit[Bridge::Suit::Heart] }
