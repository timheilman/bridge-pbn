...
require_relative '../../../../lib/portable_bridge_notation/internals/subgame_parser_factory'

module PortableBridgeNotation
  module Internals
    RSpec.describe SubgameParserFactory do
      describe('.parse') do
        context('when it is called') do
          let(:domain_builder) { double }
          let(:logger) { double }
          context('and the result is asked to handle a real tag subgame') do
            let(:parser) { described_class.make_subgame_parser(domain_builder, 'Deal') }
          ...
          context('and the result is asked to handle a nonsense tag subgame') do
            it('throws a PortableBridgeNotationError') do
              expect{described_class.make_subgame_parser(domain_builder, 'NotAValidTagName')}.
