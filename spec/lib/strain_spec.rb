...
RSpec.describe Bridge::Strain do
  ...
  describe '.forLetter' do
    it 'gives NoTrump for NT' do
      expect(described_class.for_string('NT')).to eq(Bridge::Strain::NoTrump)
    end
    it 'gives an error for a bad string' do
      expect {described_class.for_string('not a strain string')}.to raise_error(ArgumentError, /unidentifiable/)
    end
  end
